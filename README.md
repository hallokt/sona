...
### 3. Web UI（Streamlit，可选）

仓库同时提供一个轻量 Web UI（用于“跑任务 + 看状态 + 打开报告”）。

- **启动方式（Windows）**：双击仓库根目录的 `StartSonaWebUI.bat`
- **或命令行启动**：
...
说明：
- Web UI 复用了同一套工作流与会话/产物目录（`sandbox/`、`output_langgraph/` 等）。
- 若你只想用 CLI，可以忽略 Web UI。
...
## 🧪 Harness 评测（Day1~Day4 基线）

评测用例在 `tests/evals/cases/`，回放 fixture 在 `tests/fixtures/<case_id>/tools.json`。
...
# 跑 suite（replay）
python scripts/eval_runner.py --suite basic --mode replay
python scripts/eval_runner.py --suite workflow-smoke --mode replay
```
